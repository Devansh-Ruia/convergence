...
DEMO_PR = {"owner": "fastapi", "repo": "fastapi", "pr_number": 10000}
...
async def demo():
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        r = await client.post(f"{BASE_URL}/webhook/test-pr", params=DEMO_PR)
        data = r.json()
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        for agent, count in data.get("findings_count", {}).items():
            emoji = {"security": "🔒", "performance": "⚡", "testing": "🧪"}.get(
                agent, "📋"
            )
    ...
    print(
        f"PR: https://github.com/{DEMO_PR['owner']}/{DEMO_PR['repo']}/pull/{DEMO_PR['pr_number']}\n"
    )
    ...
    if response == "yes":
        async with httpx.AsyncClient() as client:
            r = await client.post(
                f"{BASE_URL}/webhook/sessions/{session_id}/post-review"
            )
            data = r.json()
